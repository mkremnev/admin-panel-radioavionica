...
import { CRow, CCard, CCardHeader, CCardBody, CForm, CFormGroup, CLabel, CInput, CFormText, CInputGroupPrepend, CInputGroup, CInputGroupText, CButton, CButtonGroup, CInputFile } from '@coreui/react';
import CIcon from '@coreui/icons-react';
import { axios } from '@/axios';

const OfficialState: { [index: string]: string } = { subunit: '', rank: '', fio: '', telephone: '' };
const unitState = { name: '', address: '', lastname: '', firstname: '', surname: '', amount: '' };

const AddUnit = () => {
	const [officialLists, setOfficialList] = useState<{ [index: string]: string }[]>([OfficialState]);
	const [unitData, setUnitData] = useState(unitState);
	...
	const onSubmit = (ev) => {
		...
		const blob = new Blob([JSON.stringify(unit)], { type: 'application/json' });
		const formData = new FormData();
		formData.append('procuration', uploadFiles, uploadFiles.name);
		formData.append('json', blob);
		axios.post('http://localhost:8081/v1/add/units', formData);
	};
				...
				<CRow>
					<CForm className="container-fluid" onSubmit={onSubmit}>
						...
						<CFormGroup>
							...
							<CInputFile onChange={(e: React.ChangeEvent<HTMLInputElement>) => setUploadFiles(e.target.files[0])} id="procuration" name="procuration" />
